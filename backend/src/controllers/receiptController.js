...
const row = (doc, label, value, y, options = {}) => {
  const { labelColor = "#4b5563", valueColor = "#1f2937", bold = false } = options;
  doc.fontSize(bold ? 11 : 10.5)
    .fillColor(labelColor).font(bold ? "Helvetica-Bold" : "Helvetica")
    .text(label, 50, y)
    .fillColor(valueColor).font(bold ? "Helvetica-Bold" : "Helvetica")
    .text(String(value ?? "-"), 0, y, { align: "right", width: 495 });
};

const badgeRow = (doc, label, badgeText, badgeColor, y) => {
  doc.fontSize(10.5).fillColor("#1f2937").font("Helvetica-Bold").text(label, 50, y);
...
const highlightBox = (doc, label, value, y, bgColor, borderColor, textColor) => {
...
const paidAmount = booking.paidAmount != null ? Number(booking.paidAmount): Number(booking.onlinePaid) || 0;   
 const travelCharge = Number(booking.travelCharge || booking.distanceCharge) || 0;
    ...
    // ── Header ──────────────────────────────────────────────
    doc.fontSize(22).font("Helvetica-Bold").fillColor("#2f2418")
      .text("Turban Culture", 50, y, { align: "center", width: 495 });
    y += 30;
    doc.fontSize(10).font("Helvetica").fillColor("#9ca3af")
      .text(type === "quick" ? "Package Booking Receipt" : "Booking Receipt",
        50, y, { align: "center", width: 495 });
    y += 22;

    dashedLine(doc, y); y += 16;

    // ── BOOKING DETAILS ─────────────────────────────────────
    sectionHeading(doc, "Booking Details", y); y += 18;
    ...
    // ── PAYMENT SUMMARY ─────────────────────────────────────
    sectionHeading(doc, "Payment Summary", y); y += 18;
    ...
    if (travelCharge > 0) {
      doc.fontSize(10.5).fillColor("#4b5563").font("Helvetica")
      ...
      doc.fontSize(8.5).fillColor(pillFg).font("Helvetica-Bold")
        .text(pillText, pillX, y + 1, { width: pillW, align: "center" });

      doc.fontSize(10.5).fillColor("#1f2937").font("Helvetica")
        .text(`₹${travelCharge}`, 0, y, { align: "right", width: 495 - pillW - 8 });
    ...
    y += 4;
    dashedLine(doc, y); y += 10;
    row(doc, "Grand Total", `₹${grandTotal}`, y, { bold: true });
    y += 26;

    highlightBox(doc, "  ✓ Amount Paid (Online)", `₹${paidAmount}`, y,
      "#f0fdf4", "#bbf7d0", "#15803d");
    y += 46;

    if (totalPending > 0) {
      highlightBox(doc, "  ⏳ Total Pending (Cash)", `₹${totalPending}`, y,
      ...
      if (pendingBase > 0) {
        ...
        doc.font("Helvetica-Bold").text(`₹${pendingBase}`, 0, y, { align: "right", width: 479 });
        doc.font("Helvetica");
        y += 14;
      }
      if (pendingTravel > 0) {
        doc.text(`   Travel charge (cash on event day)`, 60, y);
        doc.font("Helvetica-Bold").text(`₹${pendingTravel}`, 0, y, { align: "right", width: 479 });
        y += 14;
      }
      y += 6;
    }
    ...
    if (pendingBase > 0) {
      ...
      doc.fontSize(9).fillColor("#713f12").font("Helvetica-Bold")
      ...
      if (pendingTravel > 0) {
        cbRows.push([`+ Travel Charge (Cash on Event Day)`, `₹${pendingTravel}`]);
      }
